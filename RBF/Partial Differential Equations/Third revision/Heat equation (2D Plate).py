...
def pairs3D(x_tensor: torch.tensor, y_tensor: torch.tensor, t_tensor: torch.tensor):
    ...
    temp_2_x = torch.stack([pair for pair in pairs_outer if pair[0].item() == 0 or pair[0].item() == 1])
    # Removing temp_2 from outer points
    for inner_pair in temp_2_x:
        index = torch.where(torch.all(torch.eq(inner_pair, pairs_outer), dim=1))
        # Slicing is [from 0 to i and from i+1 to the end]
        pairs_outer = torch.cat((pairs_outer[:index[0].item()], pairs_outer[index[0].item() + 1:]), 0)

    # These are boundary for y = 0 & y = 1 for any time non 0
    temp_2_y = torch.stack([pair for pair in pairs_outer if pair[1].item() == 0 or pair[1].item() == 1])
    for inner_pair in temp_2_y:
        index = torch.where(torch.all(torch.eq(inner_pair, pairs_outer), dim=1))
        # Slicing is [from 0 to i and from i+1 to the end]
        pairs_outer = torch.cat((pairs_outer[:index[0].item()], pairs_outer[index[0].item() + 1:]), 0)
    ...
    temp_y = torch.stack([pos[1] for pos in temp])
    ...
    z_temp_2 = torch.full((temp_2_x.size(0) + temp_2_y.size(0),), 0., device=device)  # Stationary temp (temperature at boundary for t > 0)
...
# All points
t = torch.arange(0, 1.1, 0.1, device=device)
...
interpolator = PDE_Interpolation.InterpolatorPDE("gaussian", boundary=boundary, inner=inner, all_pairs=pairs, f=z, r=5, pde="f_y - 0.5f_xx")
...
t_RBF = torch.tensor([0.], device=device)
