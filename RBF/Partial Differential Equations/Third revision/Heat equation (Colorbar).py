...
def pairs3D(x_tensor: torch.tensor, y_tensor: torch.tensor):
    ...
    z_temp = temp_x**2 - temp_x + 0.25  # Initial temp
    z_temp_2 = torch.full((torch.stack([pos[0] for pos in temp_2]).size(0),), 0.25, device=device)  # Stationary temp
    ...
    z_outer = torch.cat((z_temp, z_temp_2), 0)  # <--- (Boundary condition)
...
boundary, inner, pairs, z = pairs3D(x, t)

interpolator = PDE_Interpolation.InterpolatorPDE("gaussian", boundary=boundary, inner=inner, all_pairs=pairs, f=z, r=4, pde="f_y - 0.075f_xx")
...
x_RBF, t_RBF = torch.meshgrid(x_RBF, t_RBF, indexing='xy')

z_RBF = interpolator.interpolate(x_RBF, t_RBF)

# --------------------------------[ Plotting 3D ]--------------------------------
fig = plt.figure()
ax = plt.axes(projection="3d")
surf = ax.plot_surface(x_RBF.cpu().detach().numpy(), t_RBF.cpu().detach().numpy(), z_RBF.cpu().detach().numpy(),
...
temp_bar = z_RBF.cpu().detach().numpy()
...
def animate(i):
    ...
    pcm = ax.pcolormesh([temp_bar[i], temp_bar[i]], cmap=cm.inferno, vmin=0, vmax=0.25)
