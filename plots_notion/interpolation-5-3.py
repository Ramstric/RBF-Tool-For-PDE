...
temperatura_alt = np.array([20, 15.5, 20.82, 19.5, 21, 19])
...
fig.add_trace(
    go.Scatter(x=np.linspace(0, 7, 300), y=temperatura_alt[1] * gaussian(np.linspace(0, 7, 300), 2, sigma), mode="lines",
               line=dict(color=custom_colors["Orange"]), showlegend=False))  # Fig 1 - Line plot

fig.add_trace(
    go.Scatter(x=np.linspace(0, 7, 300), y=temperatura_alt[3] * gaussian(np.linspace(0, 7, 300), 4, sigma), mode="lines",
...
fig.add_trace(
    go.Scatter(x=np.linspace(0, 7, 300), y=temperatura_alt[2] * gaussian(np.linspace(0, 7, 300), 3, sigma), mode="lines",
               line=dict(color=custom_colors["Green"]), showlegend=False))


fig.add_trace(
    go.Scatter(x=np.linspace(2.465, 3.5, 300), y=temperatura_alt[1] * gaussian(np.linspace(2.465, 3.5, 300), 2, sigma), mode="lines",
               line=dict(color=custom_colors["Orange"]), showlegend=False))

fig.add_trace(
    go.Scatter(x=np.linspace(2.465, 3.5, 300), y=temperatura_alt[3] * gaussian(np.linspace(2.465, 3.5, 300), 4, sigma), mode="lines",
               line=dict(color=custom_colors["Rose"]), showlegend=False))


def interpolate(x, r):
    y = 0
    for (pos, t) in zip(time, temperatura_alt):
    ...
    if x in time:
        if x == 3:
            ...
            fig.add_annotation(x=6.9, y=27, showarrow=False, ax=0, ay=0, text=f"$\large{{{y:.2f}}}$", font=dict(color=custom_colors["Green"]), bgcolor='#191919')

        if gaussian(3, x, sigma) > 0.059:
            print(3-x)
            fig.add_annotation(x=x, y=27, showarrow=False, ax=0, ay=0, text=f"$\large{{ {temperatura_alt[int(x)-1]} \cdot {gaussian(3, x, sigma):.2f}}}$",
                               font=dict(color=custom_colors[int(x)-1]), bgcolor='#191919', textangle=-30)
        else:
            fig.add_annotation(x=x, y=27, text=f"$\large{{0}}$", showarrow=False, ax=0, ay=0,
                               font=dict(color=custom_colors[int(x) - 1]), bgcolor='#191919')

        if x != 6:
            fig.add_annotation(x=x+0.5, y=27, showarrow=False, ax=0, ay=0, text="$\large{+}$", font=dict(color=custom_colors["LightGray"]), bgcolor='#191919')
        else:
            fig.add_annotation(x=x+0.5, y=27, showarrow=False, ax=0, ay=0, text="$\large{=}$", font=dict(color=custom_colors["LightGray"]), bgcolor='#191919')
...
fig.add_trace(
    go.Scatter(x=[time[1], time[2], time[3]], y=[temperatura[1], temperatura[2], temperatura[3]], mode="markers", marker=dict(color=custom_colors["Red"], size=10, symbol="x"),
               showlegend=False))
...
fig.write_image("interpolation-5-3.png", scale=2)
