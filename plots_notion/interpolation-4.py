...
def interpolate(x, r):
    ...
    if x in time:
        ...
        fig.add_trace(go.Scatter(x=[x], y=[y+0.8], mode="text", showlegend=False, text=f"+{y - temperatura[int(x) - 1]:.1f} °C", textfont=dict(color=custom_colors["Red"])))
...
fig.update_layout(template=custom, yaxis_range=[0, 27], xaxis_range=[0, 7])
...
fig.write_image("interpolation-4.png", scale=2)
