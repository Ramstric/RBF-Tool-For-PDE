...
fig.add_trace(
    go.Scatter(x=np.linspace(0, 7, 300), y=temperatura[2] * gaussian(np.linspace(0, 7, 300), 3, sigma), mode="lines",
               line=dict(color=custom_colors["Green"]), showlegend=False))  # Fig 2 - Line plot
...
def interpolate(x, r):
    ...
    if x in time:
        ...
        fig.add_annotation(x=x, y=27, showarrow=False, ax=0, ay=0, text=f"$\large{{w_{int(x)} \phi_{{x,{int(x)}}}}}$",
        ...
        if x != 6:
            ...
            fig.add_annotation(x=x+0.5, y=27, showarrow=False, ax=0, ay=0, text="$\large{=}$", font=dict(color=custom_colors["LightGray"]), bgcolor='#191919')
            fig.add_annotation(x=x+0.9, y=27, showarrow=False, ax=0, ay=0, text="$\large{f(x)}$", font=dict(color=custom_colors["LightGray"]), bgcolor='#191919')
...
fig.add_trace(
    go.Scatter(x=time_interpol, y=temperatura_interpol, mode="lines", line=dict(color=custom_colors["LightGray"]),
               showlegend=False))  # Fig 7 - Line plot
...
fig.write_image("interpolation-5.png", scale=2)
