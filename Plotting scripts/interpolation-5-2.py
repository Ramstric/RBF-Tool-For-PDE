...
def interpolate(x, r):
    ...
    if x in time:
        if x == 3:
            ...
            fig.add_annotation(x=6.9, y=27, showarrow=False, ax=0, ay=0, text=f"$\large{{{y:.2f}}}$", font=dict(color=custom_colors["Red"]), bgcolor='#191919')

        if gaussian(3, x, sigma) > 0.059:
            fig.add_annotation(x=x, y=27, showarrow=False, ax=0, ay=0, text=f"$\large{{ {temperatura[int(x)-1]} \cdot {gaussian(3, x, sigma):.2f}}}$",
...
fig.write_image("interpolation-5-2.png", scale=2)
