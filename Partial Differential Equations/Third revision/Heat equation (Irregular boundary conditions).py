...
def pairs3D(x_tensor: torch.tensor, y_tensor: torch.tensor):
    ...
    z_temp_2 = torch.full((torch.stack([pos[0] for pos in temp_2]).size(0),), 0.05, device=device)  # Stationary temp
...
interpolator = PDE_Interpolation.InterpolatorPDE("gaussian", boundary=boundary, inner=inner, all_pairs=pairs, f=z, r=5, pde="f_y - 0.02f_xx")
